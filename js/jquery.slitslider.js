	...
	$.Slitslider.prototype 	= {
		...
		_loadEvents			: function() {
			...
			if( this.slidesCount > 1 ) {
				...
				// navigate "in" or "out"
				this.$slider.on( 'advanceEvent', function( event ) {
				...
				this.$slider.on( 'goBack', function( event ) {
				...
				$(document).keydown(function(e){
					if (e.keyCode == 37) { 
					   //this.$slider.trigger('goBack');
					   _self.$slider.trigger('advanceEvent');
					}
					if (e.keyCode == 39) { 
					    _self.$slider.trigger('goBack');
					}
				});
